...
var Request = require('/HistoryCartridge/modules/server/request');
...
server.post(
    ...
    function (req, res, next) {
       var body = req.form
       var body_token = productService.getToken.call("", "");
       var token = JSON.parse(body_token.object);
       body.c_access_token = token.access_token;

       if (body) {
        var result = productService.addProductHistory.call("", body);
        res.json({ success: true, data: body.c_item_name });
       } else {
        res.json({ error: [Resource.msg('error', null)] });
    }
    next();
    }
);
